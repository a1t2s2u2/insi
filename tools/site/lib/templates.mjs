...
function siteHeader(config, sections, currentSection) {
  ...
  const graphLink = config.features.dependencyGraph
...
${mainLinks}${appendixNav}${graphLink}
        </nav>
      </div>
    </header>`;
}
...
<html lang="${escapeHtml(config.lang)}">
  ...
  <body>
    ...
    <div class="page-layout${showRefSidebar ? "" : " page-layout--no-ref"}">
...
${renderCards(mainSecs)}
      ...
      ${config.features.dependencyGraph ? `<div class="landing__group">
