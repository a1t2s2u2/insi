...
\begin{proposition}{増大領域は殻に分ける}{tb-growing-domain-shells}
$D(2)\subset D(3)\subset\cdots$ 上の非負積分の極限は、
$E_k=D(k+1)\setminus D(k)$ に分けて級数として評価する。
例えば
\[
s=x^{2/3}+y^{2/3},\qquad
D(n)=\{x,y\ge0\mid2\le s\le n\}
\]
で $|D(n)|=C(n^3-8)$ なら
\[
|E_k|=C\bigl((k+1)^3-k^3\bigr)\le C'k^2.
\]
$E_k$ では $s\ge k$ なので、$p>1$ に対し
\[
\iint_{E_k}\frac{\d x\d y}{s^3(\log s)^p}
\le\frac{C'k^2}{k^3(\log k)^p}
=\frac{C'}{k(\log k)^p}.
\]
従って前章の級数との比較で極限の収束が分かる。
\end{proposition}
