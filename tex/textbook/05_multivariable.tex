...
\begin{definition}{連続と偏微分}{tb-multivariable-basics}
$f:\R^2\to\R$ が $(a,b)$ で連続であるとは
\[
(x,y)\to(a,b)\Longrightarrow f(x,y)\to f(a,b)
\]
をいう。偏導関数は一方の変数を固定した差商
\[
f_x(a,b)=\lim_{h\to0}\frac{f(a+h,b)-f(a,b)}h,\qquad
f_y(a,b)=\lim_{h\to0}\frac{f(a,b+h)-f(a,b)}h
\]
である。全ての1階偏導関数が存在し連続なら $C^1$ 級、
全ての階数の偏導関数が連続なら $C^\infty$ 級という。
\end{definition}

$C^1$ 級なら連続であるが、偏導関数が存在するだけでは連続とは限らない。
$C^1$ 級でないことを示すには、ある偏導関数が不連続であることを一つ示せばよい。

原点での連続性は極座標 $x=r\cos\theta$, $y=r\sin\theta$ を使い、
$\abs{f(x,y)-f(0,0)}\le C r^\alpha$（$\alpha>0$）へ落とすと速い。
2021年度第1期では $\abs{\sin u}\le\min\{\abs u,1\}$ を原点付近と無限遠で使い分ける。
場合分け関数の原点での偏微分は、必ず差商の定義から計算する。
