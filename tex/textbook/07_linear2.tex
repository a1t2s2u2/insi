...
この章では、固有値を求めただけでは答案の途中である。
固有空間の簡約化、必要なら正規直交化、最後に基底を列に並べた行列まで書く。
...
A-\lambda I\sim R_\lambda,\qquad
E_\lambda=\langle v_1,\ldots,v_r\rangle
\]
である。$R_\lambda$ を省くと固有空間の計算根拠が見えない。
...
重複固有値があれば、固有値の個数だけで判断せず $\dim E_\lambda$ を簡約化で調べる。
...
\begin{proposition}{全成分1の行列を含む式}{tb-all-ones-matrix}
$e=(1,\ldots,1)^{\mathsf T}$, $J_n=ee^{\mathsf T}$ とする。
実対称行列 $A$ が
\[
A^2=aA+bJ_n+cI_n\qquad(b\ne0)
\]
を満たすなら、$AJ_n=J_nA$ であり、ある $r\in\R$ に対して
$Ae=re$ となる。さらに $r$ と異なる固有値 $\lambda$ は
\[
\lambda^2-a\lambda-c=0
\]
を満たす。従って $A$ の相異なる固有値は高々3個である。
\end{proposition}

与式に左から $A$、右から $A$ を掛けた二式を引けば
$b(AJ_n-J_nA)=O$ となる。$y=Ae$ とおくと、対称性から
$AJ_n=ye^{\mathsf T}$, $J_nA=ey^{\mathsf T}$ なので、成分比較により $y=re$ である。
$Ax=\lambda x$, $\lambda\ne r$ なら
\[
r e^{\mathsf T}x=(Ae)^{\mathsf T}x=e^{\mathsf T}Ax
=\lambda e^{\mathsf T}x
\]
より $e^{\mathsf T}x=0$、従って $J_nx=0$ である。元の式を $x$ に作用させれば二次方程式を得る。
重複度は「全重複度の和 $=n$」と $\tr A=$ 固有値の和で決める。
...
長さ2の Jordan 鎖は、まず $(A-\lambda I)v=0$ となる固有ベクトル $v$ を取り、
\[
(A-\lambda I)u=v
\]
を連立方程式として解いて作る。このとき
$Av=\lambda v$, $Au=v+\lambda u$ なので、順序付き基底 $(v,u)$ では
対応部分が $J_2(\lambda)$ になる。基底の順序を $(u,v)$ にすれば下三角になるため、
表現行列を書く前に列の順序を明記する。
...
つまり、最初に特性多項式を因数分解し、通常の場合と重根になる場合を分けてから簡約化する。
...
「不変部分空間を全て求めよ」では、まず次元 $0,1,\ldots,n$ に分ける。
0次元と全空間を忘れず、1次元は $A$、余次元1は $A^{\mathsf T}$ の固有ベクトルから得る。
...
直交対角化では、異なる固有空間同士はスペクトル定理で直交するため、
Gram--Schmidt 法は重複固有値の一つの固有空間内だけで行えばよい。
...
実対称・エルミート行列を見たら、固有値が実数、異なる固有空間が直交、
正規直交固有基底が取れる、の三点を同時に使う。
