...
\tan^{-1}x&u'=1/(1+x^2)\text{ と }\sup|u''|<\infty\text{ を使う}\\
\sec,\csc,\cot&\text{逆数の微分公式へ直す}\\
\displaystyle\sum a_n>0&\text{比の判定、または積分判定を試す}\\
\displaystyle\int f(t,x)\,\d x&\partial_t f\text{ を計算し、優関数を探す}\\
\displaystyle\int_{\phi(x)}^{\psi(x)}f(x,y)\,\d y&\text{上端}-\text{下端}+\text{中身}\\
\text{広がる領域 }D(n)&D(k+1)\setminus D(k)\text{ に分けて級数比較}\\
...
P^2=P&V=\operatorname{Im}P\oplus\ker P\text{ と }I-P\text{ を使う}\\
A^2=aA+bJ+cI&\text{左右から }A\text{ を掛けて }AJ=JA\text{ を作る}\\
...
\text{点列コンパクト}\Rightarrow\text{コンパクト}&\text{共通半径 }\delta\to\text{有限 }\delta/2\text{--net}\\
f(K)\text{ がコンパクト}&f(K)\text{ の開被覆を逆像で }K\text{ へ戻す}\\
X/{\sim}\text{ がコンパクト}&\text{コンパクトな代表集合からの全射を作る}\\
