...
抽象的な記号が出ても、有限次元の問題は基底を選べば行列と連立方程式になる。
答案では「何を未知係数としたか」「どの行列を簡約化したか」「得たベクトルが何の基底か」を残す。
...
行列の列ベクトルなら、pivot に対応する元の行列の列が像の基底、
自由変数の本数が核の次元を与える。
本数が既に空間の次元と一致するときは、一次独立か生成の片方だけ示せば他方も従う。
...
\begin{proposition}{高い冪の核は有限回で止まる}{tb-kernel-stabilization}
有限次元空間 $V$ の線形変換 $f:V\to V$ について
\[
\ker f\subset\ker f^2\subset\ker f^3\subset\cdots
\]
であり、十分大きい $m$ では $\ker f^m=\ker f^{m+1}$ となる。
\end{proposition}

包含は $f^mx=0\Rightarrow f^{m+1}x=0$ から従う。
包含が真に増えるたび次元は1以上増えるが、$\dim V$ を超えないため有限回で止まる。
一度 $\ker f^m=\ker f^{m+1}$ となれば、$f^{m+2}x=0$ に対し
$fx\in\ker f^{m+1}=\ker f^m$ だから $x\in\ker f^{m+1}$ となり、その後も等しい。

\begin{proposition}{冪等行列は射影である}{tb-idempotent-projection}
$P^2=P$ なら
\[
\boxed{V=\operatorname{Im}P\oplus\ker P},
\qquad
\operatorname{Im}P=\ker(I-P),\quad
...
実際
\[
x=Px+(I-P)x,\qquad Px\in\operatorname{Im}P,\quad
P(I-P)x=0
\]
である。共通部分の $y=Pz$ が $Py=0$ を満たせば
$y=Pz=P^2z=Py=0$ なので直和である。
像と核の二つの等式は $P(I-P)=O$ と同じ分解から従う。
固有値は $Pv=\lambda v$ に $P$ をもう一度作用させ、
$\lambda^2v=\lambda v$ とすればよい。
...
答案では「簡約化した」とだけ書かず、最終的な階段形を一つ示す。
核なら自由変数を置いた一般解まで、非斉次系なら特殊解と自由度まで書く。
...
行列式を余因子展開する前に、積・三角行列・行基本変形を使えないか見る。
$A=BC$ なら $\det A=\det B\det C$、三角行列なら対角成分の積で終わる。
行基本変形を使う場合は、行交換の符号と行の定数倍だけを追跡する。
...
行列が満たす式を与えられたら、成分計算や Gauss--Jordan 法より先に
$A$ または求めたい $cI-A$ を一方の因子としてくくれる形へ変形する。
