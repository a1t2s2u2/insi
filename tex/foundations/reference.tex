...
解答編には計算と論証の過程を記しているが、同じ定義や標準定理を問題ごとに繰り返す代わりに、
この別冊へまとめている。
...
\begin{itemize}
  \item \textbf{2026年度}: 行列の分解・エルミート行列は第\ref{ch:linear-algebra}章、誤差関数と一様収束・重積分は第\ref{ch:calculus}章、ハウスドルフ空間と連結性・集合までの距離は第\ref{ch:metric-topology}章。
...
$V$ の空でない部分集合 $W$ が\textbf{部分空間}であるとは、加法とスカラー倍で閉じていることをいう。
$v_1,\ldots,v_k\in V$ に対し、$a_1v_1+\cdots+a_kv_k$（$a_i\in K$）の形の元全体を
$\langle v_1,\ldots,v_k\rangle$ と書く。

任意の $a_1,\ldots,a_k\in K$ について $\sum_i a_iv_i=0$ から $a_1=\cdots=a_k=0$ が従うとき、
$v_1,\ldots,v_k$ は\textbf{一次独立}である。
...
\begin{theorem}{次元定理（階数・退化次数の定理）}{rank-nullity}
$V$ が有限次元なら
\[
...
\begin{definition}{行基本変形と行簡約形}{elementary-row-operations}
...
\end{enumerate}
行基本変形を繰り返して、各非零行の先頭の非零成分より下を0にした形を
\textbf{行階段形}という。さらに先頭成分を1にし、その上下も0にした形を
\textbf{簡約行階段形}という。
\end{definition}

\begin{proposition}{行基本変形は階数を保つ}{row-rank-invariance}
各行基本変形は、左から正則な基本行列を掛ける操作である。
従って行基本変形の前後で行列の階数は変わらない。
行階段形に現れる非零行の本数は、元の行列の階数に等しい。

なお、$B=A-I$ のように別の行列を引いて新しい行列を定める操作は行基本変形ではない。
行基本変形は、定めた後の $B$ の階数や連立方程式を調べるために用いる。
\end{proposition}
...
\begin{definition}{固有値・固有空間・特性多項式}{eigenvalue}
...
\begin{definition}{余因子・余因子行列の転置}{cofactor-adjugate}
$A=(a_{ij})$ を $n$ 次行列とする。$A$ から第 $i$ 行と第 $j$ 列を除いた小行列を
$A_{ij}$ とするとき
\[
C_{ij}=(-1)^{i+j}|A_{ij}|
\]
を $(i,j)$ \textbf{余因子}という。余因子行列 $(C_{ij})$ を転置した
\[
\operatorname{adj}A=(C_{ij})^{\mathsf T}
\]
を $A$ の\textbf{余因子行列の転置}という。
\end{definition}

\begin{proposition}{余因子行列による逆行列}{inverse-adjugate}
余因子展開から
\[
...
\begin{proposition}{定数係数線形漸化式の特性方程式}{recurrence-characteristic}
定数係数線形漸化式
\[
x_n+c_1x_{n-1}+\cdots+c_kx_{n-k}=0
\]
に $x_n=r^n$ を代入すると
\[
r^k+c_1r^{k-1}+\cdots+c_k=0
\]
を得る。これを\textbf{特性方程式}という。
相異なる根が $r_1,\ldots,r_k$ なら一般解は
\[
x_n=\alpha_1r_1^n+\cdots+\alpha_kr_k^n
\]
であり、係数 $\alpha_j$ は初期値を代入した連立方程式から決める。
重複度 $m$ の根 $r$ には
$r^n,nr^n,\ldots,n^{m-1}r^n$ が対応する。
\end{proposition}
...
\begin{proposition}{極化公式}{polarization-identity}
実内積空間では
\[
2\inner{x}{y}
=\norm{x+y}^2-\norm{x}^2-\norm{y}^2
\]
が成り立つ。従って線形写像 $T$ が全ての $x$ について
$\norm{Tx}=\norm{x}$ を満たすなら
\[
...
$v\ne0$ なら
\[
(vv^*)x=v(v^*x)in\langle v\rangle,
\qquad
(vv^*)v=\norm v^2v\ne0
\]
だから $\operatorname{Im}(vv^*)=\langle v\rangle$ であり、$vv^*$ の階数は1である。
\end{proposition}
...
\begin{definition}{各点収束と一様収束}{uniform-convergence}
関数列 $f_N:I\to\R$ が $f$ に\textbf{各点収束}するとは、各 $x\in I$ について
$f_N(x)\to f(x)$ となることをいう。
これに対し\textbf{一様収束}するとは
\[
\lim_{N\to\infty}\ \sup_{x\in I}\abs{f_N(x)-f(x)}=0
\]
となることをいう。すなわち誤差の上界を $x$ に依存しない量で取れることを要求する。
\end{definition}

\begin{theorem}{一様収束のもとでの項別積分}{termwise-integration}
有界閉区間 $[a,b]$ 上の連続関数列 $f_N$ が $f$ に一様収束するなら、$f$ も連続であり
\[
\lim_{N\to\infty}\int_a^bf_N(x)\,\d x=\int_a^bf(x)\,\d x.
\]
特に関数項級数が一様収束すれば項別に積分してよい。各点収束だけでは成り立たない。
\end{theorem}
...
\begin{theorem}{$S_5$ の正規部分群}{normal-subgroups-s5}
$S_5$ の正規部分群は
\[
\{1\},\qquad A_5,\qquad S_5
\]
の三つだけである。従って $S_5$ の非自明な正規部分群は $A_5$ を含み、
その位数は少なくとも60である。

これは、$A_5$ が単純群であること、$[S_5:A_5]=2$、および
$S_5$ の中心が $\{1\}$ であることから従う標準事実である。
\end{theorem}
